...
Las variables de entorno y ARG pueden afectar la caché de formas sutiles:
...
# Si APP_VERSION cambia entre builds, RUN se invalida
```
...
**Con cache mount**: los paquetes descargados persisten entre builds.
...
```bash
# Incluir metadatos de caché en la propia imagen
docker build --cache-to type=registry,ref=myrepo/myapp:latest,mode=max -t myrepo/myapp:latest .
```
...
```bash
# Build normal muestra solo si usó caché
docker build --progress=plain -t myapp . 2>&1 | grep -E '(CACHED|#)'
```

## El problema con timestamps

Docker **no** usa timestamps para determinar si un archivo cambió. Usa contenido.
Esto puede sorprender:

```bash
# Archivo con el mismo contenido = CACHED
echo "hello" > test.txt
docker build ...  # Crea capa

sleep 5
echo "hello" > test.txt  # Mismo contenido
docker build ...  # CACHED

# Archivo con contenido diferente = NUEVA capa
echo "hello world" > test.txt
docker build ...  # NUEVA capa
```

## .dockerignore y caché

Si un archivo está en `.dockerignore`, **no se copia** con `COPY . .`, por lo
que **no puede invalidar la caché**. Esta es otra razón para usar `.dockerignore`:
...
# Estos archivos NO se copian, NO afectan la caché
```

Sin `.dockerignore`, archivos como `.git` o `node_modules/` se copiarían y
invalidarían la caché aunque no sean necesarios para la imagen.
...
echo "=== Con mismo contenido (debe usar CACHE) ==="
docker build -t ts-test . 2>&1 | grep -E '\[.*/.*\]'

# Cambiar contenido
echo 'hello world' > file.txt

echo "=== Con contenido diferente (debe reconstruir) ==="
docker build -t ts-test . 2>&1 | grep -E '\[.*/.*\]'
...
# Sin cache mount (descarga desde cero cada vez)
COPY requirements.txt .
RUN pip install --no-cache-dir -r requirements.txt

# Con cache mount (persiste entre builds)
# Descomenta para ver la diferencia:
# RUN --mount=type=cache,target=/root/.cache/pip \
#     pip install --no-cache-dir -r requirements.txt

CMD ["python3", "-c", "import flask; print('Flask imported')"]
EOF

# Primer build
time docker build -t bk-cache-test . > /dev/null 2>&1

# Segundo build (debería ser rápido por pip cache local)
time docker build -t bk-cache-test . > /dev/null 2>&1

# Verificar que Flask está instalado
docker run --rm bk-cache-test

docker rmi bk-cache-test
...
│  └────────────────────────────────────────────────────────────┘ │
│                          │                                      │
│                          │ Si requirements.txt cambia...        │
│                          ▼                                      │
│  CAMBIOS FRECUENTES (abajo)                                     │
│  ┌────────────────────────────────────────────────────────────┐ │
│  │ COPY . .                    ← Solo esta capa se re-ejecuta │ │
│  │ CMD ["python3", "app.py"]                                  │ │
│  └────────────────────────────────────────────────────────────┘ │
│                                                                  │
│  INVERSIÓN = rebuild completo de todo                           │
└─────────────────────────────────────────────────────────────────┘
```

## Checklist de optimización

- [ ] Las dependencias se copian antes que el código
- [ ] `RUN pip install` está después de `COPY requirements.txt`
- [ ] `apt-get update` y `apt-get install` están en el mismo RUN
- [ ] `.dockerignore` excluye archivos innecesarios
- [ ] Las capas estáticas (FROM, RUN base) están al inicio
- [ ] `RUN` que generan caché usan `--mount=type=cache` (BuildKit)
