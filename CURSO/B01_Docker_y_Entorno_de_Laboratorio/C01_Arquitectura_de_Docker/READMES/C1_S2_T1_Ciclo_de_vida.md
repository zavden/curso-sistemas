...
- Preparar un contenedor y copiarle archivos con `docker cp` antes de iniciarlo
...
Docker usa el **cgroups freezer** para pausar. A diferencia de `kill -STOP` (que envía
una señal SIGSTOP), el freezer congela los procesos a nivel del scheduler del kernel —
los procesos simplemente dejan de ser planificados. No reciben ninguna señal, por lo que
no pueden detectar ni interceptar la pausa.
...
| 134 | 128 + 6 = SIGABRT | Abort (assert falló, panic típico) |
...
| 255 | Error de shell | Exit fuera de rango o error de ejecución |
...
handler para manejar**. Este es el comportamiento por defecto del kernel — init
debe registrar handlers para señales específicas.

En un contenedor donde PID 1 es tu aplicación (no un init real), si la aplicación
no registra un handler para SIGTERM, simplemente la ignora:
...
En un sistema Linux normal, init (PID 1) se encarga de "reap" (recoger el estado de)
procesos hijos que terminan. Si un proceso padre no llama a `wait()` para recoger
el estado de un hijo terminado, el hijo queda como "zombie" — ocupa una entrada en
la tabla de procesos pero no consume CPU ni memoria.

En un contenedor donde PID 1 es tu aplicación, si esa aplicación lanza procesos
hijos pero no los espera con `wait()`, esos hijos se convierten en zombies. No hay
un init de verdad que los limpie.

```bash
# Demostración: un contenedor que genera zombies
# El proceso crea un hijo (sleep) pero no lo espera
docker run -d --name zombie-test debian:bookworm bash -c '
  bash -c "sleep 1" &
  sleep 60
'
# Después de ~1 segundo, el proceso hijo termina y se vuelve zombie
# porque bash (PID 1) no está configurado para reap hijos
docker exec zombie-test ps aux
# Verás un proceso [sleep] en estado Z (zombie)

docker rm -f zombie-test

# Solución: usar --init para que tini haga reaping automático
docker run -d --init --name no-zombie debian:bookworm bash -c '
  bash -c "sleep 1" &
  sleep 60
'
docker exec no-zombie ps aux
# No hay zombies — tini los limpió

docker rm -f no-zombie
```

> **Nota**: Los zombies no consumen recursos significativos, pero acumulados pueden
> agotar los PIDs disponibles. En producción, siempre usa `--init` o un entrypoint
> que maneje correctamente los procesos hijos.
...
docker run -d --name ec137-noinit debian:bookworm bash -c 'while true; do sleep 1; done'
docker stop ec137-noinit
docker inspect ec137-noinit --format '{{.State.ExitCode}}'
# 137 (bash ignoró SIGTERM, docker stop envió SIGKILL tras 10s)
docker rm ec137-noinit
```
...
docker create --name file-prep debian:bookworm cat /etc/myapp.conf

# Copiar un archivo de configuración antes de iniciar
echo "setting=value" > /tmp/myapp.conf
docker cp /tmp/myapp.conf file-prep:/etc/myapp.conf

# Ahora iniciar — el contenedor verá el archivo copiado
docker start -a file-prep
# Muestra "setting=value"
