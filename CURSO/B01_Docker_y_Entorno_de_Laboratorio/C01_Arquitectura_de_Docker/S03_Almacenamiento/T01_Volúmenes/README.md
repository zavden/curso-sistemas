...
| Contenido de la imagen | Se **oculta** bajo el bind mount |
| Rendimiento | Nativo en Linux, con overhead en macOS/Windows |
...
docker run --tmpfs /tmp/cache:size=100m alpine df -h /tmp/cache

# Con --mount (más explícito)
docker run --mount type=tmpfs,destination=/tmp/cache,tmpfs-size=100m alpine df -h /tmp/cache
```
...
| Si el source no existe (bind) | **Crea el directorio** | **Error** (falla) |
...
docker run -v $(pwd)/mi_web:/usr/share/nginx/html nginx
# Solo se ve lo que hay en $(pwd)/mi_web
```
...
Los drivers más comunes en producción incluyen NFS, CIFS/SMB, y plugins específicos
de cloud (AWS EBS, Azure Disk, GCP Persistent Disk).
