...
| Inicialización | Copia contenido de la imagen la primera vez |
...
| Rendimiento | Nativo en Linux, con overhead en macOS/Windows (usa docker-desktop-data VM) |
| Permisos | Hereda permisos del host (puede causar problemas de UID/GID) |
...
**Limitaciones**:
- Solo funciona en Linux (no en Docker Desktop macOS/Windows)
- consume memoria del host
- No hay forma deinspeccionar el contenido desde el host
...
| Soporta opciones avanzadas | Limitado | Completo (mode, uid, gid, tmpfs-size, etc.) |
...
# Verás: (vacío — lo que había en la imagen está oculto)
...
| Tipo | Copia datos de la imagen? | Oculta datos de la imagen? | Comportamiento si volumen tiene datos |
|------|--------------------------|--------------------------|--------------------------------------|
| Named volume | Sí (primera vez) | No | Usa datos existentes del volumen |
| Bind mount | No | Sí (totalmente) | Lo que hay en el host es lo que se ve |
| tmpfs | N/A | No | Siempre vacío |

## Permisos y propiedad (bind mounts)

Un problema común con bind mounts es que los permisos del host pueden no coincidir
con lo que espera la aplicación en el contenedor.

```bash
# Problema típico:UID del host (1000) no existe en el contenedor
$ ls -la ./project
drwxr-xr-x  1000  staff   128 Jan 15 10:30  .

# Dentro del contenedor (que usa numeric UID):
$ docker run -v $(pwd):/app alpine ls -la /app
drwxr-xr-x  root   root   128 Jan 15 10:30  .
# Muestra como root:root porque el UID 1000 no está en /etc/passwd del contenedor
```

**Soluciones**:

```bash
# 1. Crear el usuario en el contenedor
docker run -v $(pwd):/app --user 1000:1000 alpine ls -la /app

# 2. Usar opciones de bind mount (requiere --mount)
docker run --mount type=bind,source=$(pwd),target=/app,readonly,bind-recursive=false alpine ls -la /app

# 3. Cambiar permisos en el host antes de montar
...
# Eliminar todos los huérfanos
docker volume prune

# Eliminar todos los volúmenes (cuidado!)
docker volume prune -a
```
...
### Patrones de uso compartido

```bash
# Varios lectores, un escritor
docker run -d --name db -v dbdata:/var/lib/postgresql/data postgres
docker run -d --name backup --volumes-from db backup-script

# Usar volúmenes solo para lectura
docker run -d --name web -v content:/usr/share/nginx/html:ro nginx

# Compartir código fuente (desarrollo)
docker run -d --name api -v $(pwd):/app myapi-image
docker run -d --name tests --volumes-from api test-runner
```
...
| `sshfs` | Filesystem sobre SSH |
| `glusterfs` | GlusterFS distribuido |
| `rexray` | Multi-backend (EBS, S3, etc.) |
| `vfs` | Volumen simple (sin copy-on-write, para testing) |
...
| Archivos estáticos | Named volume | `-v nginx_html:/usr/share/nginx/html` |
...
# Contenedor 2: leer datos (mientras producer corre)
sleep 2
docker run --rm -v shared-data:/data alpine cat /data/file.txt

# Contenedor 3: añadir más datos
docker run --rm -v shared-data:/data alpine sh -c 'echo "From consumer 3" >> /data/file.txt'
...
# Montar en contenedor (muestra como root:root)
docker run --rm -v /tmp/perm-test:/app alpine ls -la /app

# Con usuario específico
docker run --rm -v /tmp/perm-test:/app -u 1000:1000 alpine ls -la /app

# Cambiar permisos en el host y verificar
sudo chown 1000:1000 /tmp/perm-test/file.txt
docker run --rm -v /tmp/perm-test:/app alpine ls -la /app
...
### Ejercicio 7 — Backup de volumen

```bash
# Crear datos de prueba
docker run -d --name db -v db_backup:/data alpine \
  sh -c 'echo "important data" > /data/db.sql && echo "schema" > /data/schema.sql'
...
```bash
# Ver qué volúmenes están en uso
...
docker ps -a --format '{{.Names}}' | while read name; do
  if docker inspect --format '{{range .Mounts}}{{.Name}}{{end}}' "$name" 2>/dev/null | grep -q "mi_volumen"; then
    echo "$name usa mi_volumen"
  fi
done

# Ver tamaño de volúmenes
docker run --rm -v mi_volumen:/data alpine du -sh /data 2>/dev/null || echo "volumen no existe o está vacío"
```
