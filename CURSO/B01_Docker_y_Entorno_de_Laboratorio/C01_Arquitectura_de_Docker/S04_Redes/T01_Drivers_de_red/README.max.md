...
│  pero necesitan -p para comunicarse con el host              │
└───────────────────────────────────────────────────────────────┘
```

### Anatomía de una conexión de red

Cada vez que creas un contenedor, Docker:
...
```bash
# Ver las interfaces en el host
ip link show type veth
# veth1: ... master docker0 ...
# veth2: ... master docker0 ...

# Ver el bridge docker0
ip addr show docker0
# docker0: <BROADCAST,MULTICAST,UP> mtu 1500 qdisc noqueue state UP
#     inet 172.17.0.1/16
...
| `Options.enable_icc` | Si la comunicación entre contenedores está habilitada |
...
ip link show type veth
# veth1: ... master docker0 peer name eth0@if5
...
# ACCEPT  ...  anywhere  172.17.0.0/16
# ACCEPT  ...  172.17.0.0/16  anywhere
```
...
# El contenedor puede resolver nombres de otros contenedores
...
# También resuelve nombres de servicios de Docker Compose
```
...
# Crear contenedor conectado a ambas
...
docker exec net_test1 ping -c 2 172.17.0.3  # Usar la IP real de net_test2
...
# Útil para jobs de procesamiento offline
docker run --rm --network none -v $(pwd):/data alpine tar cf /data/backup.tar /data
```
...
# Ahora multi_net1 está en ambas redes
docker exec multi_net1 ping -c 1 multi_net2
# Funciona — están en la misma red net_b

# Desconectar
docker network disconnect net_b multi_net1
docker exec multi_net1 ping -c 1 multi_net2
# Fallará — ya no están conectados
...
### Ejercicio 7 — Configurar red bridge custom

```bash
# Red con subred específica
docker network create \
  --driver bridge \
...
docker network inspect custom_bridge --format '{{json .IPAM.Config}}' | jq '.'
