...
| Aspecto | podman-compose | Docker Compose + socket | docker compose (Docker) |
|---|---|---|---|
| Backend | Podman CLI | Podman socket | Docker daemon |
| Features completas | No | Sí | Sí |
| `depends_on` healthcheck | Parcial | Sí | Sí |
| Profiles | No | Sí | Sí |
| BuildKit cache | No | No | Sí |
| Velocidad | Lenta | Rápida | Rápida |
| Instalación extra | Sí (pip) | No | No |
| Rootless nativo | Sí | Sí | No |

## Recomendación

1. **Para desarrollo simple**: Usa `docker compose` con el socket de Podman
   (`DOCKER_HOST=unix://...`). Es la opción más compatible.

2. **Para CI/CD donde Docker no está disponible**: Usa `podman-compose`.
   La mayoría de archivos Compose funcionan.

3. **Para producción**: Considera si realmente necesitas Compose o si un
   `systemd` + Podman (o Kubernetes) es más apropiado.
...
### Ejercicio 4 — dry-run para ver comandos
...
# Ver qué comandos ejecutaría sin ejecutar
...
### Ejercicio 5 — Configurar short-names para evitarregistry questions

```bash
# Ver la configuración actual de registries
cat ~/.config/containers/registries.conf 2>/dev/null || echo "No existe"

# Crear o editar para agregar docker.io como search registry
...
# Ahora las imágenes sin prefijo deberían funcionar sin preguntar
# Prueba:
podman pull nginx:latest

# Limpiar la imagen descargada
...
│                                                                  │
│  3. Docker Engine + Podman (dual)                             │
│     docker run ... vs podman run ...                           │
