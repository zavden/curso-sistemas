...
Un servicio puede estar conectado a varias redes:
...
| Driver | Uso |
|---|---|
| `bridge` | Default. Para servicios en el mismo host |
| `host` | Elimina el aislamiento de red, usa la red del host |
| `overlay` | Para Swarm, conecta contenedores en múltiples hosts |
| `macvlan` | Asigna MAC address real a contenedores |
| `ipvlan` | Similar a macvlan pero sin MAC propia |
...
Elimina el aislamiento de red — el contenedor usa directamente la red del host:
...
services:
  monitoring:
    image: prometheus
    network_mode: host
    # No tiene puertos publicados con -p, usa los del host directamente
    # No puede usar 'ports' en Compose
```

**Cuidado**: `network_mode: host` es incompatible con `ports:` y otras
configuraciones de red de Compose.
...
# Ver información de red de un contenedor específico
docker inspect proyecto_api_1 --format '{{json .NetworkSettings.Networks}}' | python3 -m json.tool
```
...
## dns configuración por red

Se pueden configurar opciones DNS específicas para una red:
...
# Todos estos resuelven a primary-db
...
### Ejercicio 5 — Ver redes de un proyecto

```bash
mkdir -p /tmp/compose_inspect && cd /tmp/compose_inspect
...
# Ver redes por servicio
echo "=== Redes por servicio ==="
docker compose ps --format "{{.Name}}\t{{.Networks}}"

# Inspeccionar cada red
echo "=== Redes del proyecto ==="
docker network ls | grep compose_inspect

# Ver contenedores en una red específica
echo "=== Contenedores en frontend ==="
docker network inspect compose_inspect_frontend \
  --format '{{range .Containers}}{{.Name}}{{"\n"}}{{end}}'

docker compose down
cd / && rm -rf /tmp/compose_inspect
```
...
│                                                                  │
│  Importante:                                                     │
│  - Un servicio en frontend NO ve servicios en backend            │
│  - Un servicio en backend SÍ puede estar también en frontend     │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
- [ ] Servicios web públicos conectados a `frontend` y `default`
- [ ] Bases de datos solo en `backend` (no en default ni frontend)
- [ ] Servicios internos con `internal: true` si no necesitan internet
- [ ] Bases de datos y caches no exposadas con `ports:`
- [ ] Para comunicar proyectos: red externa con `external: true`
- [ ] Usar aliases para nombres amigables de servicios
