# T01 — Archivos fundamentales (Enhanced)
...
### Ejercicio 4 — Comparar passwd y shadow

```bash
#!/bin/bash
# Verificar que cada usuario en passwd tiene entrada en shadow

echo "=== Usuarios en passwd sin entrada en shadow ==="
while IFS=: read -r user rest; do
    sudo grep -q "^${user}:" /etc/shadow || echo "$user"
done < <(getent passwd | grep -v '^#')

echo ""
echo "=== Entradas en shadow sin usuario en passwd ==="
while IFS=: read -r user rest; do
    getent passwd | grep -q "^${user}:" || echo "$user"
done < <(sudo cut -d: -f1 /etc/shadow)
```

### Ejercicio 5 — UID rango

```bash
#!/bin/bash
# Clasificar todos los UIDs del sistema por rango
...
getent passwd | awk -F: '$3>0 && $3<1000{print $1": "$3}' | sort -t: -k2 -n

echo ""
echo "UIDs regulares (1000-60000):"
getent passwd | awk -F: '$3>=1000 && $3<60000{print $1": "$3}' | head -10

echo ""
echo "UID especial (65534/nobody):"
getent passwd | awk -F: '$3==65534{print $1": "$3}'
```

### Ejercicio 6 — Campos GECOS

```bash
#!/bin/bash
# Extraer nombres completos del campo GECOS

echo "=== Nombres completos de usuarios con shell bash ==="
getent passwd | grep '/bin/bash$' | while IFS=: read -r user pass uid gid gecos home shell; do
    fullname="${gecos%%,*}"
    [[ -n "$fullname" ]] && echo "$user: $fullname"
done
```

### Ejercicio 7 — Comparar distribuciones

```bash
#!/bin/bash
# Comparar configuración entre passwd y shadow de dos usuarios

compare_user() {
    local user="$1"
    echo "=== $user ==="
    getent passwd "$user"
    echo "Shadow (hash): $(sudo getent shadow "$user" | cut -d: -f2)"
    echo "Algoritmo: $(sudo getent shadow "$user" | cut -d: -f2 | cut -d'$' -f2)"
    echo ""
}
...
### Ejercicio 8 — pwck y grpck

```bash
#!/bin/bash
# Verificar consistencia del sistema de usuarios

echo "=== Verificando passwd/shadow ==="
sudo pwck

echo ""
echo "=== Verificando group/gshadow ==="
sudo grpck

echo ""
echo "=== Buscando UIDs duplicados ==="
getent passwd | awk -F: '{print $3}' | sort | uniq -d | while read -r dupuid; do
    echo "UID $dupuid aparece en:"
    getent passwd | awk -F: '$3=='"$dupuid"'{print "  "$1}'
done
```

### Ejercicio 9 — Shell que no existe

```bash
#!/bin/bash
# Encontrar usuarios cuya shell no existe

echo "=== Usuarios con shells inexistentes ==="
getent passwd | while IFS=: read -r user pass uid gid gecos home shell; do
    if [[ ! -x "$shell" && "$shell" != "/usr/sbin/nologin" && "$shell" != "/bin/false" ]]; then
        echo "$user: shell=$shell (no existe o no es ejecutable)"
    fi
done
```

### Ejercicio 10 — Archivo vs getent

```bash
#!/bin/bash
# Demostrar que getent y lectura directa de archivos dan el mismo resultado
...
echo ""
echo "=== Son idénticos? ==="
diff <(getent passwd | head -10) <(head -10 /etc/passwd) && echo "SÍ" || echo "NO"

echo ""
echo "NOTA: getent puede incluir usuarios de LDAP/NIS que NO aparecen en /etc/passwd"
```
