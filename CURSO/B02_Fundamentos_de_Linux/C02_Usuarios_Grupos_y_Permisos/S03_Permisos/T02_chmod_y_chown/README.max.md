...
| `chgrp` | No | Sí | No | Solo grupos a los que perteneces |
| `chattr` | No | No | No | Atributos de filesystem |
...
### Ejercicio 4 — chmod X mayúscula

```bash
#!/bin/bash
# Comparar chmod -R 755 vs chmod -R a+X

mkdir -p /tmp/testx/{subdir,scripts}
echo "contenido" > /tmp/testx/file.txt
echo "#!/bin/bash" > /tmp/testx/scripts/script.sh

echo "=== Con chmod -R 755 ==="
chmod -R 755 /tmp/testx
find /tmp/testx -type f -exec stat -c '%a %n' {} \;

echo ""
echo "=== Con chmod -R a+X ==="
chmod -R a+X /tmp/testx
find /tmp/testx -type f -exec stat -c '%a %n' {} \;
# ¿Cambió algo en los archivos?

rm -rf /tmp/testx
```

### Ejercicio 5 — chown y symlinks

```bash
#!/bin/bash
# Symlinks: -h vs sin -h

echo "test" > /tmp/target.txt
ln -s /tmp/target.txt /tmp/mylink

echo "=== Antes de chmod ==="
ls -la /tmp/mylink /tmp/target.txt

echo ""
echo "=== chmod 000 sobre el link (sin -h) ==="
chmod 000 /tmp/mylink
ls -la /tmp/mylink /tmp/target.txt

echo ""
echo "=== chmod 600 sobre el link (con -h) ==="
chmod 600 /tmp/mylink
ls -la /tmp/mylink /tmp/target.txt

rm /tmp/target.txt /tmp/mylink
```

### Ejercicio 6 — chown a usuario que no existe

```bash
#!/bin/bash
# Intentar cambiar owner a un usuario inexistente

touch /tmp/test.txt
chown nobodyx /tmp/test.txt 2>&1 || echo "Error esperado"

rm /tmp/test.txt
```

### Ejercicio 7 — chmod go= y chmod o=

```bash
#!/bin/bash
# Diferencia entre go= y o=

touch /tmp/test1 /tmp/test2

chmod 776 /tmp/test1
chmod go= /tmp/test1   # group y others = nada
chmod 776 /tmp/test2
chmod o= /tmp/test2    # solo others = nada

echo "go= : $(stat -c '%a' /tmp/test1)"
echo "o=  : $(stat -c '%a' /tmp/test2)"

rm /tmp/test1 /tmp/test2
```

### Ejercicio 8 — chattr inmutable

```bash
#!/bin/bash
# Probar chattr para hacer archivo inmutable

touch /tmp/immutable.txt
chmod 644 /tmp/immutable.txt

echo "=== Antes de chattr +i ==="
lsattr /tmp/immutable.txt

sudo chattr +i /tmp/immutable.txt

echo ""
echo "=== Después de chattr +i ==="
lsattr /tmp/immutable.txt

echo ""
echo "=== Intentar modificar (debería fallar) ==="
echo "nuevo" > /tmp/immutable.txt 2>&1 || echo "Denegado (esperado)"

echo ""
echo "=== Intentar chmod (debería fallar) ==="
chmod 777 /tmp/immutable.txt 2>&1 || echo "Denegado (esperado)"

echo ""
echo "=== root también falla ==="
sudo sh -c 'echo "nuevo" > /tmp/immutable.txt' 2>&1 || echo "Denegado (esperado)"

sudo chattr -i /tmp/immutable.txt
rm /tmp/immutable.txt
```

### Ejercicio 9 — chmod 0 y recuperación

```bash
#!/bin/bash
# chmod 000 y cómo root puede recuperar

touch /tmp/locked.txt
chmod 000 /tmp/locked.txt

echo "Permisos: $(stat -c '%a' /tmp/locked.txt)"

echo ""
echo "=== Usuario normal no puede leer ==="
cat /tmp/locked.txt 2>&1 || echo "Denegado (esperado)"

echo ""
echo "=== root SÍ puede leer ==="
sudo cat /tmp/locked.txt && echo "Leído por root (esperado)"

echo ""
echo "=== root puede cambiar permisos ==="
sudo chmod 644 /tmp/locked.txt
stat -c 'Permisos恢复了: %a' /tmp/locked.txt

rm /tmp/locked.txt
```

### Ejercicio 10 — Directorio sin x

```bash
#!/bin/bash
# w sin x vs r sin x vs x sin r

mkdir -p /tmp/perms-test
touch /tmp/perms-test/file.txt

for perms in "200" "400" "100"; do
    chmod $perms /tmp/perms-test
    echo "=== chmod $perms (${perms//0/-}) ==="
    echo "  ls /tmp/perms-test: $(ls /tmp/perms-test/ 2>&1 | head -1)"
    echo "  stat file.txt: $(stat /tmp/perms-test/file.txt 2>&1 | head -1)"
done

chmod 755 /tmp/perms-test
rm -rf /tmp/perms-test
```

### Ejercicio 11 — Permisos efectivos con ACLs

```bash
#!/bin/bash
# Cuando chmod modifica la máscara ACL

touch /tmp/acl-test.txt
setfacl -m u:nobody:rw /tmp/acl-test.txt

echo "=== Con ACL de nobody:rw ==="
getfacl /tmp/acl-test.txt

echo ""
echo "=== chmod g=r (modifica la máscara, no el grupo) ==="
chmod g=r /tmp/acl-test.txt
getfacl /tmp/acl-test.txt
# nobody ahora tiene efectivo r-- por la máscara
