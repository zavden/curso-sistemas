...
| l | Multithreaded |
| + | Foreground |
| < | Alta prioridad (nice<0) |
| N | Baja prioridad (nice>0) |
| L | Paginas bloqueadas (mlock) |
...
```bash
#!/bin/bash
# Encontrar los 10 procesos más antiguos aún corriendo

ps -eo pid,etime,cmd --sort=-etime | head -11
```

### Ejercicio 5 — Threads de un proceso

```bash
#!/bin/bash
# Mostrar threads de un proceso

PID=${1:-$$}
echo "Threads del proceso $PID:"
ps -o pid,nlwp,tid,cmd -L -p $PID
```

### Ejercicio 6 — pgrep y pgrep

```bash
#!/bin/bash
# Comparar pgrep -l vs -a

echo "=== pgrep -l (nombre corto) ==="
pgrep -l sshd

echo ""
echo "=== pgrep -a (línea completa) ==="
pgrep -a sshd
```

### Ejercicio 7 — Procesos por sesión

```bash
#!/bin/bash
# Agrupar procesos por session ID

echo "Sesiones activas:"
ps -eo sid,pid,cmd --sort=sid | awk '{print "SID "$1": PID "$2" "$NF}'
```

### Ejercicio 8 — Format personalizado avanzado

```bash
#!/bin/bash
# Formato estilo "ps aux" pero personalizado

ps -eo user:12,pid:6,%cpu:6,%mem:6,vsz:8,rss:8,stat:4,args \
    --sort=-%cpu \
    | head -15
```

### Ejercicio 9 — Estado D (I/O wait)

```bash
#!/bin/bash
# Buscar procesos en estado D y mostrar qué están esperando

echo "Procesos en Disk Sleep (D):"
ps -eo pid,stat,wchan:20,cmd | awk '$2 ~ /D/'
```

### Ejercicio 10 — Comparar ps y pgrep

```bash
#!/bin/bash
# Diferencia entre pgrep -x (exacto) y pgrep sin -x

echo "=== pgrep bash ==="
pgrep bash | head -5

echo ""
echo "=== pgrep -x bash (exacto) ==="
pgrep -x bash | head -5
```
