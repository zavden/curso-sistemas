...
```
  ...
  Swp[                    0K/8.00G]
   ...
   1842 dev       20   0  437M 42.8M 31.0M S  2.3   0.3  0:15.42 /usr/bin/gnome-shell
     502 root      20   0  112M 21.3M 15.6M S  0.7   0.1  0:08.23 /usr/lib/systemd/systemd-journald
```
...
## Tabla: comandos interactivos esenciales

| Tecla en top | Tecla en htop | Acción |
|---|---|---|
| P/M/T/N | F6 | Ordenar por CPU/MEM/TIME/PID |
| k | F9 | Matar proceso |
| r | F7/F8 | Renice |
| u | F4 | Filtrar por usuario |
| 1 | F2 | CPUs individuales |
| H | H | Mostrar threads |
| c | c | Comando completo |
| V | F5 | Tree view |
| q | q | Salir |
...
### Ejercicio 4 — Captura con timestamp

```bash
#!/bin/bash
# Capturar top cada 5 segundos durante 1 minuto

for i in {1..12}; do
    echo "=== Muestra $i - $(date) ==="
    top -bn1 -o %CPU | head -12
    sleep 5
done > /tmp/top-capture.txt

echo "Captura guardada en /tmp/top-capture.txt"
```

### Ejercicio 5 — Load average y CPUs
...
echo "CPUs: $CPUS"
echo "Load avg (1min): $LOAD"
echo "Utilización: $(echo "scale=2; $LOAD / $CPUS * 100" | bc)%"

if (( $(echo "$LOAD / $CPUS > 1.0" | bc -l))); then
    echo "ALERTA: Sistema sobrecargado"
fi
```

### Ejercicio 6 — top vs htop memoria

```bash
#!/bin/bash
# Comparar cómo top y htop muestran la memoria

echo "=== top (líneas de resumen) ==="
top -bn1 | head -5

echo ""
echo "=== htop (barras) ==="
which htop &>/dev/null && htop -d 0 -C --tree || echo "htop no está instalado"
```

### Ejercicio 7 — Filtrar por usuario en top
...
USER=${1:-$(whoami)}
echo "Monitoreando a $USER"
top -bn1 -u "$USER" -o %CPU | head -15
```

### Ejercicio 8 — Encontrar procesos %CPU alto

```bash
#!/bin/bash
# Scripts que encuentran procesos que consumen CPU

echo "=== Procesos con más de 5% CPU ==="
top -bn1 -o %CPU | awk '$9 > 5 {print}'

echo ""
echo "=== Sumar %CPU total ==="
top -bn1 -o %CPU | tail -n +7 | awk '{sum += $9} END {print "Total CPU: " sum "%"}'
```

### Ejercicio 9 — top y %CPU instantáneo vs acumulado

```bash
#!/bin/bash
# Mostrar que %CPU en top es instantáneo

# Iniciar un proceso CPU-bound
(while true; do :; done) &
PID=$!

echo "PID $PID started. Observa %CPU en top (debería subir a ~100%)"
echo "Presiona Ctrl+C para matar"
sleep 2
kill $PID 2>/dev/null
```

### Ejercicio 10 — Alternativas (btop/glances)

```bash
#!/bin/bash
# Verificar qué monitores están instalados

for cmd in top htop btop glances atop; do
