...
| Archivo | Contenido | Legible por otros |
|---|---|---|
| cmdline | Línea de comando completa | Sí |
| comm | Nombre corto del ejecutable | Sí |
| status | Estado legible (UIDs, memoria, threads) | Sí |
| stat | Estado crudo para parsing | Sí |
| environ | Variables de entorno | No |
| exe | Symlink al binario | Sí |
| cwd | Symlink al directorio de trabajo | Sí |
| root | Symlink a la raíz del namespace | Sí |
| fd/ | File descriptors abiertos | No |
| maps | Mapa de memoria | Sí |
| smaps_rollup | Resumen de memoria | Sí |
| limits | Límites del proceso | Sí |
...
### Ejercicio 4 — fd y tipos de archivos

```bash
#!/bin/bash
# Clasificar los fds de un proceso

PID=${1:-$$}
echo "=== File descriptors de PID $PID ==="
...
### Ejercicio 5 — UIDs 4-tupla

```bash
#!/bin/bash
# Mostrar los 4 UIDs y GIDs

echo "=== UIDs ==="
grep Uid /proc/$$/status | awk '{print "Real: "$2" Effective: "$3" Saved: "$4" Filesystem: "$5}'
echo ""
echo "=== GIDs ==="
grep Gid /proc/$$/status | awk '{print "Real: "$2" Effective: "$3" Saved: "$4" Filesystem: "$5}'
```

### Ejercicio 6 — Context switches

```bash
#!/bin/bash
# Ver context switches de un proceso

echo "=== voluntary_ctxt_switches vs nonvoluntary ==="
grep -E 'ctxt_switches' /proc/$$/status

# voluntary: el proceso cede CPU voluntariamente (sleep, I/O)
# involuntary: el scheduler lo preemptó
```

### Ejercicio 7 — PSS vs RSS

```bash
#!/bin/bash
# Comparar RSS y PSS de un proceso

PID=${1:-$$}
echo "RSS: $(grep Rss /proc/$PID/smaps_rollup)"
echo "PSS: $(grep Pss /proc/$PID/smaps_rollup)"
echo ""
echo "PSS es menor porque divide memoria compartida proporcionalmente"
```

### Ejercicio 8 — maps con permisos

```bash
#!/bin/bash
# Encontrar regiones de memoria ejecutables (para análisis de seguridad)

echo "=== Regiones ejecutables (r-x) ==="
grep ' r-xp ' /proc/$$/maps

echo ""
echo "=== Regiones escribibles (rw-p) ==="
grep ' rw-p ' /proc/$$/maps | head -10
```

### Ejercicio 9 — namespaces IDs

```bash
#!/bin/bash
# Comparar namespaces entre el host y un PID

TARGET_PID=${1:-$$}
echo "=== Namespaces del proceso $TARGET_PID ==="
ls -la /proc/$TARGET_PID/ns/ | awk '{print $NF}'
...
### Ejercicio 10 — /proc/[pid]/stack

```bash
#!/bin/bash
# Ver la pila de llamadas del kernel (si está disponible)

PID=${1:-$$}

if [[ -r /proc/$PID/stack ]]; then
    echo "=== Stack de PID $PID ==="
    cat /proc/$PID/stack
else
    echo "/proc/$PID/stack no es legible"
fi
```
