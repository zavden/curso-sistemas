...
# Los programas les asigna el significado que quieran
...
| Señal | Número | Uso más común | Capturable? |
|---|---|---|---|
| SIGTERM | 15 | Terminación limpia (kill default) | Sí |
| SIGKILL | 9 | Terminación forzada | No |
| SIGINT | 2 | Ctrl+C | Sí |
| SIGQUIT | 3 | Ctrl+\ (core dump) | Sí |
| SIGHUP | 1 | Recargar config / terminal cerrado | Sí |
| SIGSTOP | 19 | Detener proceso | No |
| SIGTSTP | 20 | Ctrl+Z | Sí |
| SIGCONT | 18 | Reanudar proceso detenido | N/A (no se captura) |
| SIGUSR1/2 | 10/12 | Definido por la aplicación | Sí |
| SIGCHLD | 17 | Notificación de hijo terminado | Ignorar por defecto |
| SIGPIPE | 13 | Pipe roto | Sí |
| SIGSEGV | 11 | Fallo de memoria | No (genera core dump) |
...
### Ejercicio 4 — Proceso que ignora señales

```bash
#!/bin/bash
# Crear un proceso que ignora SIGTERM

trap 'echo "Ignorando SIGTERM" TERM; :' TERM
echo "PID=$$ — ignora SIGTERM, acepta SIGINT"
while true; do sleep 1; done
```

### Ejercicio 5 — orphan y re-parenting

```bash
#!/bin/bash
# Crear un proceso huérfano y observar re-parenteo

bash -c 'sleep 300 & echo "hijo=$!"; exit' &
sleep 1
echo "Sleep corriendo como PID $(pgrep -f 'sleep 300')"
echo "Su padre es $(ps -o ppid= -p $(pgrep -f 'sleep 300'))"
# Debe ser 1 (systemd/init adoptó el huérfano

pkill -f 'sleep 300'
```

### Ejercicio 6 — Nombres vs números

```bash
#!/bin/bash
# kill -l para convertir entre nombres y números

echo "SIGTERM = $(kill -l TERM)"
echo "15 = $(kill -l 15)"
echo ""
echo "Todas las señales:"
kill -l | tr ' ' '\n' | nl
```

### Ejercicio 7 — Ver señales pendientes

```bash
#!/bin/bash
# Ver señales que puede manejar un proceso

PID=${1:-$$}
echo "Señales que $PID puede capturar (no son SIGKILL ni SIGSTOP):"
kill -l | tr ' ' '\n' | while read sig; do
    # Intentar trapear cada señal
    trap "echo $sig" "$sig" 2>/dev/null && echo "$sig capturable"
done
```

### Ejercicio 8 — trap en bash

```bash
#!/bin/bash
# Demo de trap para capturar señales

trap 'echo "Recibí SIGUSR1"; exit 0' USR1
trap 'echo "Recibí SIGTERM"; exit 0' TERM

echo "PID=$$ — esperando señales USR1 o TERM"
while true; do sleep 1; done
```

### Ejercicio 9 — Ver si proceso está esperando señales

```bash
#!/bin/bash
# Ver el wchan (función del kernel donde duerme un proceso)

PID=${1:-$$}
echo "El proceso $PID está esperando en:"
cat /proc/$PID/wchan 2>/dev/null || echo "no legible o corriendo"

echo ""
echo "Detalles del estado:"
cat /proc/$PID/status | grep -E 'State|State:'
```

### Ejercicio 10 — orphan vs zombie vs normal

```bash
#!/bin/bash
echo "=== Normal: padre alive ==="
sleep 100 &
sleep_pid=$!
cat /proc/$sleep_pid/status | grep State

echo ""
echo "=== Zombie: padre no hace wait ==="
# El hijo terminaría zombie solo si el padre no hace wait
# Demo conceptual:
bash -c 'sleep 100 & exec sleep 1000' &
# El primero sleep termina, queda zombie si el shell no hace wait

echo ""
echo "=== Orphan: padre murió ==="
bash -c 'sleep 100 & exit' &
orphaned_pid=$!
sleep 1
echo "Sleep $orphaned_pid huérfano — padre PPID=$(ps -o ppid= -p $orphaned_pid)"
# Debería ser 1

pkill -f 'sleep 100' 2>/dev/null
```
