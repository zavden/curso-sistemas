...
```bash
# 1. Imprimir solo las líneas 1 a 5 de /etc/passwd:
...
### Ejercicio 4 — In-place con backup
...
# Editar con backup
sed -i.bak 's/line/ROW/' /tmp/test.txt

# Ver qué quedó
cat /tmp/test.txt
cat /tmp/test.txt.bak

rm /tmp/test.txt /tmp/test.txt.bak
```

---

### Ejercicio 5 — Direccionamiento por rango

```bash
# Eliminar líneas 5 a 10
sed '5,10d' /etc/passwd | head -5

# Eliminar todo excepto líneas 1-3
sed '1!d' /etc/passwd

# Agregar texto después de línea 3
sed '3a\--- INSERTED LINE ---' /etc/passwd | head -5
```

---

### Ejercicio 6 — Eliminar comentarios y vacías

```bash
# De un archivo de configuración con comentarios
...
sed '/^[[:space:]]*#/d; /^[[:space:]]*$/d' /tmp/config.txt
...
### Ejercicio 7 — multiline con N

```bash
# Juntar líneas que terminan en ,
echo -e "one,\ntwo,\nthree" | sed 'N; s/,\n/, /'
# one, two
# three

# Dividir campos key=value
echo -e "a=1\nb=2\nc=3" | sed 's/\(.\)=\(.\)/\1 => \2/'
# a => 1
# b => 2
# c => 3
...
### Ejercicio 8 — Sustitución con &

```bash
# Envolver matches en tags
echo "hello world" | sed 's/[a-z]*/<&>/g'
# <hello> <world>

# Duplicar números
echo "abc 123 def 456" | sed -E 's/[0-9]+/[&,&]/g'
# abc 123,123 def 456,456
```

---

### Ejercicio 9 — Transformar CSV a tabla

```bash
# CSV a columnas alineadas
...
### Ejercicio 10 — hold space para reverse

```bash
# Reversa de palabras en una línea
echo "one two three" | sed -E 's/(.)/\1\n/g' | tac | tr -d '\n'; echo
# three two one
```
