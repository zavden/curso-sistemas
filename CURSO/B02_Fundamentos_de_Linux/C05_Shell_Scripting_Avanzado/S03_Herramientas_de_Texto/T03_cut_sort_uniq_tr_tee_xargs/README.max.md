...
| `-V` | version | Version sort (1.1, 1.10, 1.2) |
| `-z` | zero-terminated | Para archivos con newlines en nombres |
| `-o` | output | Escribir a archivo (seguro para inplace) |
...
### Ejercicio 4 — tee para debug

```bash
# Guardar salida intermedia mientras se procesa
echo "test data" | tee /tmp/debug.txt | tr 'a-z' 'A-Z' | tee /tmp/debug2.txt
cat /tmp/debug.txt
cat /tmp/debug2.txt
rm /tmp/debug.txt /tmp/debug2.txt
```

---

### Ejercicio 5 — tr para limpieza
...
echo -e "hello\x00world" | tr -d '\000'

# Crear histogram de caracteres
echo "hello" | fold -w1 | sort | uniq -c
```

---

### Ejercicio 6 — sort con múltiples claves
...
# Ordenar por campo 2 (numérico), luego por campo 1
sort -t: -k2,2n -k1,1 /tmp/test.txt
...
### Ejercicio 7 — xargs paralelo

```bash
# Simular procesamiento paralelo
echo {1..10} | xargs -n 1 -P 4 sh -c 'echo "Job {} started"; sleep 0.1; echo "Job {} done"'
```

---

### Ejercicio 8 — Pipeline de análisis de logs

```bash
# Simular access log
cat > /tmp/access.log << 'EOF'
192.168.1.1 GET /index.html
10.0.0.1 GET /api/users
...
# Requests por método
awk '{ print $2 }' /tmp/access.log | sort | uniq -c
...
### Ejercicio 9 — cut vs awk para campos

```bash
# Datos de prueba
echo "a::b::c::d" > /tmp/test.txt

# cut no puede con ::
# cut -d:: -f2 /tmp/test.txt  # ERROR

# awk sí puede
awk -F'::' '{ print $2 }' /tmp/test.txt

# multiple delimiters consecutive
...
### Ejercicio 10 — uniq sin sort (awk)

```bash
# Eliminar duplicados preservando orden original (sin sort)
echo -e "a\nb\na\nc\nb" | awk '!seen[$0]++'

# vs sort | uniq (reordena)
echo -e "a\nb\na\nc\nb" | sort | uniq
```
