...
| `-w` | word | Palabra completa |
| `-x` | line | Línea completa |
| `-v` | invert | Líneas que NO coinciden |
| `-F` | fixed | Búsqueda literal sin regex |
...
# Mostrar error con 2 líneas de contexto
grep -C 2 ERROR /tmp/test.log

# Mostrar solo lo que viene DESPUÉS del error
grep -A 2 ERROR /tmp/test.log

# Mostrar solo lo que viene ANTES del error
...
### Ejercicio 5 — Invertir matches

```bash
# Mostrar líneas que NO contienen comentarios ni vacías
grep -vE '^#|^$' /etc/fstab

# Mostrar líneas que NO tienen números
grep -vE '[0-9]' file
```

---

### Ejercicio 6 — Solo la parte que coincide

```bash
# Extraer emails
echo "Contact: john.doe@example.com or jane@corp.org" | \
    grep -oE '[[:alnum:]._+-]+@[[:alnum:].-]+\.[[:alpha:]]{2,}'

# Extraer números con punto decimal
echo "Price: 19.99 USD, discount 5.5%" | grep -oE '[0-9]+\.[0-9]+'
```

---

### Ejercicio 7 — word boundary

```bash
# Buscar "the" pero no "there", "them", etc.
echo -e "the cat\nthere is a cat\nthem\ncat" | grep -w 'the'
...
### Ejercicio 8 — Archivos sin pattern

```bash
# Encontrar archivos que NO contienen TODO
grep -rL 'TODO' src/

# Verificar que un string NO existe en ningún archivo
grep -r 'FUTURE_FEATURE' . || echo "No future features found"
```

---

### Ejercicio 9 — Máximo de matches

```bash
# Solo las primeras 10 líneas con "error"
grep -m 10 'error' /var/log/syslog
```

---

### Ejercicio 10 — Combinación con otras herramientas

```bash
# Líneas únicas con patrón
ps aux | grep -v grep | grep 'python' | awk '{ print $2 }' | sort -u
