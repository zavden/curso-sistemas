...
### Ejercicio 4 — Processing log

```bash
# Simular log
cat > /tmp/test.log << 'EOF'
2024-03-17 10:00:00 INFO Starting service
2024-03-17 10:00:01 ERROR Connection failed
2024-03-17 10:00:02 WARN Retry attempt 1
2024-03-17 10:00:03 ERROR Connection failed
2024-03-17 10:00:04 INFO Service started
EOF

# Contar errores
awk '/ERROR/ { errors++ } END { print "Errors:", errors }' /tmp/test.log

# Mostrar solo líneas con ERROR
...
### Ejercicio 5 — Promedio y totales

```bash
# Calcular promedio de una columna
echo -e "10\n20\n30\n40\n50" > /tmp/nums.txt
awk '{ sum += $1; count++ } END { print "Average:", sum/count }' /tmp/nums.txt
rm /tmp/nums.txt
```

---

### Ejercicio 6 — AWK como calculadora

```bash
# Operaciones matemáticas en awk
awk 'BEGIN {
    print "2^10 =", 2^10
    print "sqrt(2) =", sqrt(2)
    print "log(e) =", log(exp(1))
    print "random =", rand()
    print "int(random * 100) =", int(rand() * 100)
}'
```

---

### Ejercicio 7 — Reemplazar valores
...
### Ejercicio 8 — Parsear CSV con manejo de comas

```bash
# CSV con comillas
echo '"Smith, John",30,New York' | \
    awk -F'","' '{ 
        gsub(/"/, "", $1)
        print "Name:", $1, "Age:", $2, "City:", $3 
    }'
```

---

### Ejercicio 9 — FNR vs NR

```bash
# Comparar FNR y NR con múltiples archivos
echo "file1 contents" > /tmp/f1.txt
echo "file2 contents" > /tmp/f2.txt
...
### Ejercicio 10 — Invertir campos

```bash
# Imprimir campos en orden inverso
