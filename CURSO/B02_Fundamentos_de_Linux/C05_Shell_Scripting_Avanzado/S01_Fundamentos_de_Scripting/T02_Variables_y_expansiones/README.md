...
```bash
#!/bin/bash
# Guardar como /tmp/test-defaults.sh
...
# Probar:
# bash /tmp/test-defaults.sh                     → ¿qué error da?
# bash /tmp/test-defaults.sh 3000 localhost dev   → ¿qué imprime?
# bash /tmp/test-defaults.sh 3000                 → ¿qué error da?
```
...
# Sin usar basename, dirname, sed ni awk:
