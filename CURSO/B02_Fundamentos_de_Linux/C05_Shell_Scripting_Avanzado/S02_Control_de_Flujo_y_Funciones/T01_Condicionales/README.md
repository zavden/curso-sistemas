...
| Word splitting | Sí (citar variables) | No |
...
```bash
# BUG: falta espacio después de [ y antes de ]
[-f /etc/passwd]
...
```bash
# Escribir una condición que verifique si $1 es un número entero positivo:
INPUT="${1:-abc}"

if [[ $INPUT =~ ^[0-9]+$ ]]; then
    echo "$INPUT es un número entero positivo"
else
...
```bash
# Dado un archivo, identificar su tipo por extensión:
FILE="${1:-test.txt}"
