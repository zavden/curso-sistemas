...
| `FUNCNAME[@]` | Array con nombres de funciones en el call stack (main primero) |
| `BASH_SOURCE[@]` | Array con rutas de archivos en el call stack |
| `BASH_LINENO[@]` | Array con números de línea del call stack |
| `BASH_ARGC[@]` | Array con conteo de argumentos de cada función |
| `BASH_SUBSHELL` | Nivel de anidamiento de subshells |
| `SECONDS` | Segundos desde el inicio del script |
...
|---|---|---|---|
| `set -x` | Flujo de ejecución | Runtime | Solo trace, noisy |
| `bash -n` | Errores de sintaxis | Estático | No detecta lógica ni typos |
| `shellcheck` | Bugs, malas prácticas, portabilidad | Estático | No entiende intención |
| `trap DEBUG` | Comando antes de ejecutar | Runtime | Overhead alto |
| `trap ERR` | Error y stack trace | Runtime | Solo en `set -e` |
| `$DEBUG` var | Prints condicionales | Runtime | Requiere código |
...
### Ejercicio 4 — Stack trace completo
...
# Observar cómo el stack muestra la cadena de llamadas
# Probar:
# ./stack_trace.sh
```

### Ejercicio 5 — Script con tracing separado de stderr

```bash
#!/usr/bin/env bash
set -euo pipefail

TRACE_FILE="${TRACE_FILE:-/tmp/trace_$$.log}"
exec 3>"$TRACE_FILE"
BASH_XTRACEFD=3

export PS4='+[${BASH_SOURCE[0]##*/}:${LINENO}] '

echo "Este es stdout (no se tracea)"
echo "Este es stderr (no se tracea)" >&2
...
cat "$TRACE_FILE"
echo ""
echo "=== stdout/stderr separados del trace ==="

# Probar:
# ./trace_separate.sh
# diff <(grep "^+" "$TRACE_FILE") <(cat "$TRACE_FILE" | grep "^+")
```

### Ejercicio 6 — ShellCheck: encontrar yfix errores

```bash
#!/bin/bash
# Archivo con errores intencionales - corregir con ayuda de shellcheck

cat << 'SCRIPT' > /tmp/checkme.sh
#!/bin/bash
files=$(ls *.txt)
for f in $files; do
    echo "Procesando $f"
...
# Ejecutar shellcheck yfix cada problema:
shellcheck /tmp/checkme.sh

echo ""
echo "=== Corrigiendo el archivo ==="
# Aplicar fixes sugeridos manualmente
cat << 'FIXED' > /tmp/checkme_fixed.sh
#!/bin/bash
...
shellcheck /tmp/checkme_fixed.sh
...
breakpoint() {
    echo "=== BREAKPOINT en ${BASH_SOURCE[1]}:${LINENO} ===" >&2
...
### Ejercicio 8 — Medición de tiempo de ejecución

```bash
#!/usr/bin/env bash
set -euo pipefail

export PS4='+[${BASH_SOURCE[0]##*/}:${LINENO} $(date "+%H:%M:%S.%N")] '
...
time_section() {
    ...
    echo "=== $label took $(echo "$end - $start" | bc) segundos ===" >&2
}

time_section "Sleep 1 segundo" sleep 1
time_section "Echo simple" echo "hello"
time_section "Loop 100" for i in $(seq 1 100); do :; done

# Probar:
# ./timed_trace.sh
```

### Ejercicio 9 — Debug con valores en cada paso
...
watch_vars() {
    ...
    echo "[WATCH${BASH_SUBSHELL}]${output}" >&2
}
...
echo "Antes: x=$x y=$y"
calculate x y
echo "Después: result=$result"

# Probar:
# ./var_watch.sh
```

### Ejercicio 10 — Error handler avanzado con dump de estado
...
dump_state() {
    ...
    echo "Línea error: ${BASH_LINENO[0]}" >&2
    echo "Función: ${FUNCNAME[1]:-main}" >&2
    echo "Comando fallido: $BASH_COMMAND" >&2
    echo "Exit code: $?" >&2
    echo "" >&2
    echo "=== Variables del scope ===" >&2
    for var in "${!ARR_@}" "${!CONFIG_@}" VAR1 VAR2 DATA; do
        [[ "${!var}" ]] && echo "  $var=${!var}" >&2
    done 2>/dev/null || true
    echo "" >&2
    echo "=== Memoria (vars) ===" >&2
    compgen -v | head -20 | while read -r v; do
        [[ "${!v}" ]] && echo "  $v=${!v}" >&2
    done
}
...
### Ejercicio 11 — tracepipe: grep traces en tiempo real
...
(export PS4='+[${LINENO}] '
 ...
 sleep 1
 echo "test"
) 2>&1 | grep -E '\[[0-9]+\]|^\+' | head -20

# Comparar con trace sin filtrar:
echo "=== Con filtro ==="
(export PS4='+[${LINENO}] '
 set -x
 sleep 1
