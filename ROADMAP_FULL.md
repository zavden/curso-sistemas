# Roadmap Completo — Linux · C · Rust · Go
...
**Ruta SysAdmin/Infra:**
... → B08 → B09 → B10 → B11 → B22 (Go)
B22 puede cursarse tras B09; se lista al final porque complementa la ruta
con el lenguaje dominante del ecosistema SysAdmin (Docker, K8s, Terraform, Prometheus).
...
**Ruta DevOps/SRE:**
B01 → B02 → B09 → B22 → B18 → B19 → B20 → B21
B22 (Go) se recomienda antes de B18 para poder escribir herramientas propias
en Go además de scripts Bash/Python. Especialmente útil antes de B20 (K8s).

**Ruta Go / Tooling SysAdmin:**
B01 → B02 → B22 → B09 → B18 → B20 → B21
Ruta corta para quien quiere Go + infraestructura sin pasar por C/Rust.
Go es el lenguaje en el que están escritos Docker, Kubernetes, Terraform,
Prometheus, Grafana, etcd, containerd, Consul, Vault, y la mayoría de
...
> Go es el lenguaje dominante del ecosistema SysAdmin/DevOps: Docker, Kubernetes,
> Terraform, Prometheus, Grafana, etcd, containerd, Consul y Vault están escritos
> en Go. Dominar Go permite leer el código fuente de estas herramientas, escribir
> operadores de Kubernetes, plugins de Terraform, exporters de Prometheus, y CLIs
> de infraestructura profesionales.
...
**S02 — Compilación y Cross-compilation**
...
- [ ] T03 — Build tags y ldflags (//go:build, -ldflags "-X main.version=...", inyección de metadatos)
...
- [ ] T03 — Conversiones de tipo (casting explícito obligatorio, no coerción implícita, strconv package)
- [ ] T04 — Tipo rune (Unicode code points, iteración por runes vs bytes, strings.Builder)

### C03 — Control de Flujo `[A]`

**S01 — Condicionales y Bucles**
...
- [ ] T03 — switch (sin fall-through por defecto, fallthrough keyword, switch sin expresión, type switch)
- [ ] T04 — Labels y goto (break/continue con labels, goto raro pero existe, cuándo es legítimo)

**S02 — Defer, Panic y Recover**
- [ ] T01 — defer (LIFO, evaluación de argumentos, defer en loops ¡cuidado!, patrones de cleanup)
- [ ] T02 — panic y recover (cuándo usar panic, recover en deferred functions, no usar como excepciones)
...
**S02 — Closures y Funciones Avanzadas**
- [ ] T01 — Closures (captura de variables, closure sobre loop variable pre/post Go 1.22, goroutine closures)
- [ ] T02 — init() (función especial, orden de ejecución, múltiples init por archivo, cuándo usarla)
- [ ] T03 — Métodos (value receiver vs pointer receiver, method sets, cuándo usar cada uno)

### C05 — Tipos Compuestos `[A]`
...
- [ ] T03 — Slicing y gotchas (s[low:high:max], compartición de array subyacente, memory leaks)
...
- [ ] T04 — Comparación (== en structs solo si campos comparables, reflect.DeepEqual, maps package)
...
- [ ] T03 — nil interface vs interface con valor nil (la trampa clásica, interface(nil) ≠ (*T)(nil))
- [ ] T04 — Comparación con Rust (interfaces vs traits, implícita vs explícita, method sets vs impl blocks)

### C07 — Manejo de Errores `[A]`

**S01 — El Patrón de Go**
- [ ] T01 — Error como valor (el tipo error interfaz, errors.New, fmt.Errorf, por qué no excepciones)
- [ ] T02 — El patrón if err != nil (idiomático, named returns para cleanup)
...
- [ ] T03 — Comparación con Rust (error vs Result<T,E>, wrapping vs From trait, ausencia de ?)

### C08 — Concurrencia `[A]`

**S01 — Goroutines**
- [ ] T01 — Goroutines (go keyword, modelo M:N goroutines sobre OS threads, scheduler de Go)
- [ ] T02 — Goroutines vs threads (coste ~2KB stack vs ~1MB, crecimiento dinámico, scheduling)
...
- [ ] T02 — Once, Map, Pool (sync.Once singleton, sync.Map map concurrente, sync.Pool reciclaje)
- [ ] T03 — Context (context.Background, WithCancel, WithTimeout, WithValue, propagación de cancelación)
- [ ] T04 — Comparación con Rust (goroutines vs tokio tasks, channels vs mpsc, Mutex vs Mutex<T>)

### C09 — Paquetes e I/O `[A]`

**S01 — Sistema de Paquetes**
...
- [ ] T03 — Encoding (encoding/json Marshal/Unmarshal/tags/Decoder streaming, encoding/csv, encoding/xml)
- [ ] T04 — Línea de comandos (os.Args, flag package, cobra tercero, stdin/stdout/stderr)

### C10 — Networking y HTTP `[A]`
...
### C11 — Genéricos y Testing `[A]`

**S01 — Genéricos (Go 1.18+)**
- [ ] T01 — Funciones genéricas (type parameters, constraints, comparable, any)
- [ ] T02 — Tipos genéricos (structs y métodos con type parameters, limitaciones actuales)
